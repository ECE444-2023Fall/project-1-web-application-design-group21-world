...
def test_organizer_insert(client):
    ...
    test_organizer = dict(organizer_name="test organizer 01", organizer_email="testorganizer01@gmail.com")

    response = client.post("/organizer/create", json=test_organizer, follow_redirects=True)
    assert response.status_code == 200
    with client:
         organizer = Organizer.query.filter_by(organizer_name=test_organizer["organizer_name"]).first()
...
def test_organizer_insert_multiple(client):
    """Ensure that organizers can be added to Organizer Table"""
    test_organizer = dict(organizer_name="test organizer 02", organizer_email="testorganizer02@gmail.com")
