...
class UserDetailsChangeForm(FlaskForm):
    ...
    faculty = SelectField(
        ...
        choices=[
            ...
            ("Architecture", "Architecture, landscape and design"),
            ("ArtSci", "Arts and Science"),
            ("ContStd", " Continuing studies"),
            ("dentistry", "Dentistry"),
            ("Edu", "Education"),
            ("Info", "Information"),
            ("Kinesology", "Kinesiology and physical education"),
            ("Law", "Law"),
            ...
            ("Anthro", "Anthroplogy"),
            ("Bio", "Biology"),
            ("ChemandPhy", "Chemical and Physical Sciences"),
            ("Eco", "Economics"),
            ...
            (
                "ICCIT",
                "Instituute of Communication, Culture, Information and Technology (ICCIT)",
            ),
            ("Lang", "Language Studies"),
            ("Management", "Management"),
            ("Math", "Mathematical and Computational Sciences"),
            ("Philo", "Philosophy"),
            ("PolSci", "Political Science"),
            ("Psych", "Psychology"),
            ("Socio", "Sociology"),
            ("VisStd", "Visual Studies"),
            ("Arts", "Arts"),
            ("Sci", "Science"),
        ],
...
class OrganizerSignupForm(FlaskForm):
    organization_name = StringField(
        "What is your organization name?", validators=[DataRequired()]
    )
    organization_email = EmailField(
        "What is your organization UofT email address?",
        validators=[DataRequired(), Email()],
    )
    ...
    organization_description = StringField(
        "Organization Description (*)", validators=[Length(max=500)]
    )
    organization_website_link = StringField(
        "Website Link", validators=[URL(), Optional()]
    )
    organization_instagram_link = StringField(
        "Instagram Link", validators=[URL(), Optional()]
    )
    organization_linkedin_link = StringField(
        "LinkedIn Link", validators=[URL(), Optional()]
    )
...
class EventForm(FlaskForm):
    ...
    description = StringField(
        "Event Description", validators=[Optional(), Length(max=500)]
    )
    date = DateField(
        "Date of Event", format="%Y-%m-%d", validators=[DataRequired()]
    )
    ...
    google_map_link = StringField(
        "Map of Event", validators=[Optional(), URL()]
    )
    fee = StringField(
        "Fee for Event", validators=[Optional()]
    )  # ,NumberRange(min=1, max=500)])
    # has_rsvp = StringField("RSVP Button?", validators=[DataRequired()])
    external_registration_link = StringField(
        "Link of Registration", validators=[Optional(), URL()]
    )
    submit = SubmitField("Submit")
...
class eventInterestForm(FlaskForm):
    interests = MultiCheckboxField(
        "Select Your Event's Target Interest Area",
        choices=[],
        validators=[Optional()],
        coerce=int,
    )
